...
pv <- ggplot() +
  ...
  theme_minimal() +
  xlab(" ") +
  ylab(expression("PV+ Cells/1000" * mm^2))+
  theme(
...
pvgenplot <- ggplot() +
  ...
  geom_jitter(data = Image_data3,
              aes(x = Genotype, y = Green.Puncta.Density, color = Genotype),
...
pvgenplot
ggsave("PV_puncta_genotype_density.png", plot = pvgenplot,
       width = 2, height = 4, dpi = 300)
```

```{r}
my_colors <- c("NR" = "chartreuse3",
               "MD" = "yellow")
summary_df_PV <- Image_data3 %>%
  group_by(Condition) %>%
  summarise(
...
pvconplot <- ggplot() +
  ...
  geom_errorbar(data = summary_df_PV,
                ...
                position = position_dodge(width = 0.5)) +  # match dodge width
  geom_jitter(data = Image_data3,
              aes(x = Condition, y = Green.Puncta.Density, color = Condition),
              width = 0.15, size = 2, alpha = 0.7) +
...
pvconplot
ggsave("PV_puncta_condition_density.png", plot = pvconplot,
       width = 2, height = 4, dpi = 300)
```
...
wfa <- ggplot() +
  ...
  theme_minimal() +
  xlab(" ") +
  ylab(expression("WFA+ Cells/1000" * mm^2)) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"  # Remove legend
  ) +
...
coloc <- ggplot() +
  ...
  theme_minimal() +
  xlab(" ") +
  ylab(expression("PV+/WFA+ cells/1000" * mm^2)) +
...
coloc_con <- ggplot() +
  ...
  geom_errorbar(data = summary_df_Coloc,
                ...
                position = position_dodge(width = 0.5)) +  # match dodge width
  geom_jitter(data = Image_data3,
              aes(x = Condition, y = Coloc.Puncta.Density, color = Condition),
              width = 0.15, size = 2, alpha = 0.7) +
...
coloc_con
ggsave("coloc_puncta_condition_density.png", plot = coloc_con,
       width = 2, height = 4, dpi = 300)
```
...
percentcoloc <- ggplot() +
  ...
  theme_minimal() +
  xlab(" ") +
  ylab("Percent Colocalized Cells") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"  # Remove legend
  ) +
...
Percent_data <- Image_data2 |>
  group_by(Animal, Genotype, Condition) |>
  summarise(
    percent_coloc = (mean(coloc_percent)*100)
  )
my_colors <- c("WT" = "dodgerblue",
               "KO" = "firebrick1")

summary_df_percent <- Percent_data %>%
  group_by(Genotype) %>%
  summarise(
    MeanCount = mean(percent_coloc, na.rm = TRUE),
    SEM = sd(percent_coloc, na.rm = TRUE) / sqrt(n()),
    .groups = "drop"
  )

# Plot
colocpercentplotgt <- ggplot() +
  geom_col(data = summary_df_percent,
           aes(x = Genotype, y = MeanCount, fill = Genotype),
           alpha = 0.5, width = 0.4, position = position_dodge(width = 0.5)) +  # key change
  geom_errorbar(data = summary_df_percent,
                aes(x = Genotype, y = MeanCount, ymin = MeanCount - SEM, ymax = MeanCount + SEM),
                width = 0.15,
                color = "black",
                position = position_dodge(width = 0.5)) +  # match dodge width
  geom_jitter(data = Percent_data,
              aes(x = Genotype, y = percent_coloc, color = Genotype),
              width = 0.15, size = 2, alpha = 0.7) +

  theme_minimal() +
  xlab(" ") +
  ylab(expression("%(PV+/PNN+)/PV+")) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"
  ) +
...
colocpercentplotgt
ggsave("colocpercentplotgty.png", plot = colocpercentplotgt,
       width = 2, height = 4, dpi = 300)
```

```{r}
Image_data2$Condition <- factor(Image_data2$Condition, levels = c("NR", "MD"))
...
summary_df_percent_con <- Percent_data %>%
  group_by(Condition) %>%
  summarise(
    MeanCount = mean(percent_coloc, na.rm = TRUE),
    SEM = sd(percent_coloc, na.rm = TRUE) / sqrt(n()),
...
# Plot
colocpercentplotcon <- ggplot() +
  geom_col(data = summary_df_percent_con,
           aes(x = Condition, y = MeanCount, fill = Condition),
           alpha = 0.5, width = 0.4, position = position_dodge(width = 0.5)) +  # key change
  geom_errorbar(data = summary_df_percent_con,
                aes(x = Condition, y = MeanCount, ymin = MeanCount - SEM, ymax = MeanCount + SEM),
                width = 0.15,
                color = "black",
                position = position_dodge(width = 0.5)) +  # match dodge width
  geom_jitter(data = Percent_data,
              aes(x = Condition, y = percent_coloc, color = Condition),
              width = 0.15, size = 2, alpha = 0.7) +
...
colocpercentplotcon
ggsave("colocpercentplotcon.png", plot = colocpercentplotcon,
       width = 2, height = 4, dpi = 300)
```

```{r}
modelper = lm(`Green.Puncta.Density` ~ Condition * Genotype,
           data = Image_data3)
summary(modelPV2)
