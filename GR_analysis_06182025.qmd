...
p <- ggplot() +
  ...
  # Bracket for "MD"
  geom_segment(aes(x = 0.7, xend = 4.3, y = -1, yend = -1)) +  # horizontal line
  geom_segment(aes(x = 0.7, xend = 0.7, y = -1, yend = -0.8)) +  # left vertical
  geom_segment(aes(x = 4.3, xend = 4.3, y = -1, yend = -0.8)) +  # right vertical
  annotate("text", x = 2.5, y = -1.2, label = "MD", size = 4) +  # label

  # Bracket for "NR"
  geom_segment(aes(x = 4.7, xend = 8.3, y = -1, yend = -1)) +
  geom_segment(aes(x = 4.7, xend = 4.7, y = -1, yend = -0.8)) +
  geom_segment(aes(x = 8.3, xend = 8.3, y = -1, yend = -0.8)) +
  annotate("text", x = 6.5, y = -1.2, label = "NR", size = 4) +

  # Expand plot space downward
  coord_cartesian(ylim = c(-2, max(summary_df_WFA$MeanCount) * 2))
...
p <- ggplot() +
  ...
  # Expand plot space downward
  coord_cartesian(ylim = c(-2, max(summary_df_coloc$MeanCount) * 2))
